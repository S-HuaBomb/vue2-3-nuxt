...
import { getSliders } from "@/common/api/sliders";
...
export default {
  ...
  methods: {
    ...
    //鼠标移入
    async mourseHover(item) {
      this.isShow = true;
      //获取课程标签
      this.querycourse.entity.firstCategory = item.id;
      let resCourseTags = await getCourseTags(this.querycourse);
      this.courseTags = resCourseTags.data.pageInfo.list;

      //查询课程
      let resCourseSearch = await courseSearch(this.querycourse);
...
.navSwiper {
  ...
  width: 100%;
  height: 640px;
  background: url(/public/image/transitionbg.png);
}
